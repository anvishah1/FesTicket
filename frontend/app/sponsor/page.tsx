    ...
    return (
      <main className="min-h-screen bg-[#fdfdff]">
        ...
        <div className="max-w-2xl mx-auto px-6 py-16">
          <div className="bg-white rounded-2xl border border-[#C5BAC4] p-8 text-center shadow-lg">
            <div className="w-20 h-20 mx-auto mb-6 rounded-full bg-green-100 flex items-center justify-center">
              <svg className="w-10 h-10 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
              </svg>
            </div>
            <h1 className="text-2xl font-bold text-[#29104A] mb-3">Thank You for Your Sponsorship!</h1>
            ...
            <div className="bg-[#F9F7FA] rounded-xl p-4 mb-6 text-left">
              <p className="text-sm text-[#6B597F] mb-2">Reference Details:</p>
              <p className="font-semibold text-[#29104A]">{formData.companyName}</p>
              <p className="text-sm text-[#6B597F]">{formData.email}</p>
              {uploadedFiles.length > 0 && (
                <p className="text-xs text-[#6B597F] mt-2">{uploadedFiles.length} file(s) uploaded</p>
              )}
            </div>
            <button
              onClick={() => router.push("/")}
    ...
    <main className="min-h-screen bg-[#fdfdff] text-[#29104A]">
      ...
      <div className="max-w-3xl mx-auto px-6 py-8">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-[#29104A] mb-2">Become a Sponsor</h1>
          <p className="text-[#6B597F]">Partner with us and showcase your brand to thousands of attendees</p>
        </div>

        {/* Main Form */}
        <div className="bg-white rounded-2xl border border-[#C5BAC4] overflow-hidden shadow-lg">
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-8">
            ...
            <div>
              <h3 className="text-sm font-semibold text-[#29104A] mb-4 flex items-center gap-2">
            ...
            <div>
              ...
              <div className="space-y-4">
                {/* Upload Area */}
                <div className="border-2 border-dashed border-[#C5BAC4] rounded-xl p-6 text-center hover:border-[#522C5D] transition-colors bg-[#F9F7FA]">
                  <input
                    type="file"
                    id="file-upload"
                    multiple
                  ...
                  <label htmlFor="file-upload" className="cursor-pointer">
                    <div className="w-14 h-14 mx-auto mb-3 rounded-full bg-[#522C5D]/10 flex items-center justify-center">
                      <svg className="w-7 h-7 text-[#522C5D]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                      </svg>
                    </div>
                    <p className="text-[#29104A] font-medium mb-1">Click to upload files</p>
                    <p className="text-sm text-[#6B597F]">or drag and drop</p>
                ...
                {uploadedFiles.length > 0 && (
                  <div className="space-y-2">
                    <p className="text-sm font-medium text-[#6B597F]">Uploaded Files ({uploadedFiles.length})</p>
                    {uploadedFiles.map((file, index) => (
                      <div
                        key={index}
                        className="flex items-center justify-between p-3 bg-[#F9F7FA] rounded-lg border border-[#C5BAC4]"
                      >
                        <div className="flex items-center gap-3">
                          {getFileIcon(file.type)}
                          <div>
                            <p className="text-sm font-medium text-[#29104A] truncate max-w-[200px]">{file.name}</p>
                            <p className="text-xs text-[#6B597F]">{formatFileSize(file.size)}</p>
                          </div>
                        </div>
                        <button
                          type="button"
            ...
            <div className="pt-4 border-t border-[#C5BAC4]">
              ...
              <p className="text-center text-xs text-[#6B597F] mt-3">
