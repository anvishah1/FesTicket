...
export default function PasswordFields({
  ...
  return (
    <div className="space-y-4">
      <div>
        <label htmlFor={`${idPrefix}-new`} className="block text-sm font-medium text-[var(--text-strong)] mb-1">
          {newLabel}
        </label>
        <div className="relative">
          <input
            ...
            className="w-full px-3 py-2 pr-16 border rounded-md focus:ring-2 focus:ring-[var(--ring-plum)] focus:border-[var(--border-plum)]"
          ...
          <button
            ...
            className="absolute inset-y-0 right-0 px-3 text-sm text-[var(--text-soft)] hover:text-[var(--text-strong)]"
          >
      ...
      <div>
        <label htmlFor={`${idPrefix}-confirm`} className="block text-sm font-medium text-[var(--text-strong)] mb-1">
          {confirmLabel}
        </label>
        <div className="relative">
          <input
      ...
      <ul className="space-y-1" aria-label="Password requirements">
        {checks.rules.map((rule) => (
          <li
            key={rule.label}
            className={`flex items-center gap-2 text-sm ${rule.ok ? "text-green-700" : "text-[var(--text-soft)]"}`}
          >
