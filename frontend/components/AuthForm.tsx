...
export default function AuthForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // mock success -> redirect or update state
    alert("Mock sign-in success (replace with real auth)");
  }

  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      {/* Google sign-in (placeholder) */}
      <SocialButton onClick={() => alert("Mock: Google sign-in (replace)")}>
      ...
      <div className="flex items-center justify-between text-sm">
        ...
        <a className="text-primary-600 hover:underline" href="/forgot">Forgot password?</a>
      </div>
      ...
      <div className="mt-4">
        <a
          href="/signup"
          className="w-full inline-flex items-center justify-center gap-2 rounded-lg px-4 py-3 font-medium transition bg-primary-500 hover:bg-primary-600 text-white"
        >
